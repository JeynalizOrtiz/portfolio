---
title: "Final Report"
subtitle: "EVR628 data visualization project"
...
This project contains data relevant to sea turtle hatchling disorientation events documented 
during the 2025 sea turtle nesting season. Data showcases all disorientations in
beaches located within 5 Miami-Dade County Beaches: Key Biscayne, Fisher Island, Miami Beach, 
Haulover, and Golden Beach. The dataset captures spatial locations, species identity, and the magnitude of each disorientation event recorded by trained survey personnel.

# Objectives

The objective of this analysis is to quantify hatchling disorientation events per site and visualize their spatial distribution using GPS coordinates. This includes summarizing nest counts by species and exploring spatial patterns in disorientation hotspots.
...
Raw variables include date, nest code, latitude, longitude, species, disorientation direction, and number of turtles disoriented. The cleaned dataset includes an additional variable that assigns each nest to its corresponding survey site. The following discusses this more in depth:
...
C###RBH - Label assigned to relocated nests from the 2025 Bal Harbour 
          ...
          nest number, followed by "R" for relocation and "BH" for Bal Harbour. 
...
The dataset was cleaned by renaming columns, correcting a GPS export error, and rounding latitude and longitude values to consistent decimal places to address differences between Trimble and Google Earth outputs, both of which are GPS methods applied in the Miami-Dade
County Parks Sea Turtle Conservation Program data collecting. Site names were assigned based on nest codes, a date entry error was corrected, and the final cleaned dataset was saved as an RDS file for subsequent analyses.
...
Across all surveyed sites, disorientation events and nest counts varied substantially, with Miami Beach showing the highest number of disorientations (n = 89), followed by Haulover (n = 47) and Key Biscayne (n = 30). Fisher Island had the fewest disorientations (n = 4). Across all surveyed sites, Miami Beach exhibited the highest number of disorientation events (`r dis_per_site$nests[dis_per_site$sites=="miami beach"]`), followed by Haulover (`r dis_per_site$nests[dis_per_site$sites=="haulover"]`) and Key Biscayne (`r dis_per_site$nests[dis_per_site$sites=="key biscayne"]`). Green turtles (*Chelonia mydas*) contributed a smaller but consistent portion of nests at every site, while leatherback (*Dermochelys coriacea*) nesting occurred only at Golden Beach, Haulover, and Miami Beach, and in relatively low numbers. When accounting for each site’s total nesting activity, loggerheads comprised the majority of nests at most sites—for example, 75% of nests at Fisher Island and 73% at Key Biscayne—whereas greens accounted for roughly 17–40% depending on location.

```{r}
#| fig-cap: "Disorientation count by site during the 2025 nesting season."
...
```{r}
#| fig-cap: "Heat map of nests by site and species."
...
map <- function(data = fl_shp, x, bb, pad = 0.05, title = "title") {
  ggplot() +
  geom_sf(data,
          fill = "gray95",
          color = "black",
...
```{r}
#| fig-cap: "Map of Key Biscayne hatchling disorientation points overlaid on the Florida state boundary shapefile."
...
```{r}
#| fig-cap: "Map of Haulover hatchling disorientation points overlaid on the Florida state boundary shapefile."
...
```{r}
#| fig-cap: "Map of Fisher Island hatchling disorientation points overlaid on the Florida state boundary shapefile."
...
```{r}
#| fig-cap: "Map of Miami Beach hatchling disorientation points overlaid on the Florida state boundary shapefile."
...
```{r}
#| fig-cap: "Map of Golden Beach hatchling disorientation points overlaid on the Florida state boundary shapefile."
...
Appelhans, T., Detsch, F., Reudenbach, C., & Woellauer, S. (2025). *mapview: Interactive viewing of spatial data in R *(Version 2.11.4) Rpackage. https://CRAN.R-project.org/package=mapview

Dunnington, D. (2025). *ggspatial: Spatial data framework for ggplot2* (Version 1.1.10) Rpackage. https://CRAN.R-project.org/package=ggspatial

Firke, S. (2024). *janitor: Simple tools for examining and cleaning dirty data* (Version 2.2.1) 
Rpackage. https://CRAN.R-project.org/package=janitor

Florida Department of Transportation. (2019). Detailed Florida state boundary. https://gis-fdot.opendata.arcgis.com/datasets/fdot::detailed-florida-state-boundary/about

Grolemund, G., & Wickham, H. (2011). Dates and times made easy with lubridate. *Journal of Statistical Software, 40*(3), 1–25. https://www.jstatsoft.org/v40/i03/

Wickham, H. (2016). *ggplot2: Elegant graphics for data analysis*. Springer.

Miami-Dade County Parks Sea Turtle Conservation Program. (2025). 2025 sea turtle nesting season disorientation dataset. Unpublished raw data. Miami-Dade County.

Pebesma, E., & Bivand, R. (2023). *Spatial data science: With applications in R*. Chapman and Hall/CRC. https://doi.org/10.1201/9780429459016

Pebesma, E. (2018). Simple features for R: Standardized support for spatial vector data. *The R Journal, 10*(1), 439–446. https://doi.org/10.32614/RJ-2018-009

Pedersen, T. (2025). *patchwork: The composer of plots* (Version 1.3.2) 
Rpackage. https://CRAN.R-project.org/package=patchwork

R Core Team. (2025). *R: A language and environment for statistical computing*. R Foundation for Statistical Computing. https://www.R-project.org/

Villasenor-Derbez, J. C. (2025). *EVR 628: Introduction to data management and visualization for environmental scientists*. Department of Environmental Science and Policy, University of Miami Rosenstiel School of Marine Atmospheric, and Earth Science.
